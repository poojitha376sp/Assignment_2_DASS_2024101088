...
class Player:
    ...
    def net_worth(self):
        ...
        properties_value = sum(
            prop.financials["price"] for prop in self.properties if not prop.is_mortgaged
        )
    ...
    def move(self, steps):
        ...
        # If current position is less than old position, we have wrapped around (passed Go)
        # Exception: Landing exactly on 0 is also passing Go.
        if self.position < old_position or (self.position == 0 and steps > 0):
    ...
    def status_line(self):
        ...
        jail_tag = " [JAILED]" if self.jail_info["in_jail"] else ""
        return (
